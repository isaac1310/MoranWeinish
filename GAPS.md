...
| 13 | ~~**TravelHub: 7 screen exports**~~ **Done 2026-09-10 — no export needed.** The seven screens are drawn components in the `.fig`, so they were rebuilt as inline SVG straight from the decoded node tree (`122:2528, 2894, 3373, 6091, 6280, 6395, 6923`). They scale to any size and cost ~30 KB gzipped for all seven; only the photos and map tiles inside them are real files (`assets/img/travelhub/screen-*`). Original note:  | Section "07 / The interface" shows seven phone screens that are Figma-drawn components, not images. Please export each at 2× as PNG (632×1292) into `assets/img/travelhub/` with these exact names: `screen-01-trips-home.png`, `screen-02-trip-detail.png`, `screen-03-itinerary-day.png`, `screen-04-itinerary-map.png`, `screen-05-budget.png`, `screen-06-bookings.png`, `screen-07-group.png`. Figma: select the phone "Background" frame inside each Container component → Export → PNG 2x. Until then the site shows labelled placeholders. |

| 14 | ~~**Suzuki: icon set export**~~ **Done 2026-09-10 — no export needed.** Frame 1739340856 was rendered from the decoded vectors to `assets/img/suzuki/icons.png` (1432x82, 32 KB). Original note:  | Section "06 / Design system → Iconography" shows nine Hebrew-labelled service icons drawn as vectors. Export the icon row (Frame 1739340856, 716×41) as PNG 2× to `assets/img/suzuki/icons.png`. Placeholder shown until then. |
